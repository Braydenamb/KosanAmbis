...
export default function Sidebar({ isMobileMode = false }) {
  const {
    focusHours,
    waterIntake,
    setWaterIntake,
  ...
  return (
    <aside className={`w-full flex transition-all duration-300 ease-in-out ${isMobileMode ? 'flex-row overflow-x-auto snap-x [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-2 items-stretch gap-4' : 'flex-col gap-4 lg:gap-6 overflow-hidden items-stretch'}`}>
      
      {/* 0. BRAND CARD */}
      <div className={`hidden glass-card relative group transition-all duration-300 ease-in-out w-full p-6 flex-col gap-4 overflow-hidden`}>
        ...
        <div className="flex items-center w-full min-w-0 transition-all duration-500 ease-in-out gap-4 justify-start">
          <div className="transition-all duration-500 shrink-0 flex items-center justify-center p-3 bg-brand-500/10 border border-brand-500/20 rounded-2xl text-brand-600 shadow-sm">
            <Shield className="w-6 h-6" />
          </div>
          <div className="transition-all duration-500 ease-in-out min-w-0 opacity-100 max-w-xs translate-x-0">
      ...
      {/* 1. STATUS MODE */}
      <div className={`glass-card flex flex-col transition-all duration-300 ease-in-out ${isMobileMode ? 'min-w-[15.5rem] snap-center shrink-0 p-4 gap-3' : 'w-full p-5 gap-3'}`}>
      ...
      <div className={`glass-card flex flex-col transition-all duration-300 ease-in-out ${isMobileMode ? 'min-w-[15.5rem] snap-center shrink-0 p-4 gap-3' : 'w-full p-5 gap-3'}`}>
        ...
        <div className="w-full bg-slate-200/50 rounded-full h-1.5 overflow-hidden mt-1 border border-white/50">
          <div className="bg-brand-500 h-full rounded-full transition-all duration-500" style={{ width: `${Math.min((streakDays / 7) * 100, 100)}%` }} />
        </div>
      ...
      <div className={`glass-card flex flex-col transition-all duration-300 ease-in-out ${isMobileMode ? 'min-w-[15.5rem] snap-center shrink-0 p-4 gap-3 justify-between' : 'w-full p-5 gap-4'}`}>
        ...
        <div className="flex items-center justify-between bg-white/40 p-2.5 rounded-xl border border-white/60 w-full">
          <button onClick={() => handleWaterChange(-1)} className="p-1 hover:bg-slate-200 rounded-lg text-rose-500 hover:text-rose-600 active:scale-90 transition-all cursor-pointer">
          ...
          <button onClick={() => handleWaterChange(1)} className="p-1 hover:bg-slate-200 rounded-lg text-brand-600 hover:text-brand-700 active:scale-90 transition-all cursor-pointer">
            <Plus className="w-4 h-4" />
          </button>
        </div>

        <div className="w-full bg-slate-200/50 rounded-full h-1.5 overflow-hidden border border-white/50">
          <div className="bg-blue-500 h-full rounded-full transition-all duration-500 ease-out" style={{ width: `${Math.min((waterIntake / 8) * 100, 100)}%` }} />
        </div>
      </div>
      ...
      <div className={`glass-card flex flex-col transition-all duration-300 ease-in-out ${isMobileMode ? 'min-w-[15.5rem] snap-center shrink-0 p-4 gap-3 justify-between' : 'w-full p-5 gap-4'}`}>
        ...
        <div className="flex items-center justify-between bg-white/40 p-2.5 rounded-xl border border-white/60 w-full">
          <button onClick={() => handleSleepChange(-0.5)} className="p-1 hover:bg-slate-200 rounded-lg text-rose-500 hover:text-rose-600 active:scale-90 transition-all cursor-pointer">
            <Minus className="w-4 h-4" />
          </button>
          <div className="text-center">
            <div className="text-xl font-extrabold text-slate-800 leading-none font-mono">
          ...
          <button onClick={() => handleSleepChange(0.5)} className="p-1 hover:bg-slate-200 rounded-lg text-brand-600 hover:text-brand-700 active:scale-90 transition-all cursor-pointer">
            <Plus className="w-4 h-4" />
          </button>
        </div>
