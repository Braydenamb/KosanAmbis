    ...
    <div className="glass-card-no-hover p-6 md:p-8 flex flex-col gap-6 md:gap-8 border border-white/85">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          <div className="bg-slate-50/70 border border-slate-200/60 p-5 rounded-2xl flex flex-col gap-4 transition-all duration-300">
            {/* Phone Stats */}
            <div className="flex items-center justify-between gap-3 min-w-0">
              <div className="flex items-center gap-3 min-w-0">
                <div className="p-3 bg-brand-500/10 rounded-xl text-brand-600 shrink-0">
                  <Smartphone className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <div className="text-sm font-bold text-slate-800 truncate">Xiaomi 13 Pro</div>
                  <span className="text-[10px] font-bold text-slate-400 font-mono block truncate">
                    {phoneOnline ? '🟢 ONLINE (KDE CONNECT)' : '🔴 OFFLINE'}
                  </span>
                </div>
              </div>
              <div className="text-right shrink-0">
                <div className="text-sm font-black text-slate-850">{phoneBattery}%</div>
                <div className="w-16 bg-slate-200 h-1.5 rounded-full mt-1.5 overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-3 min-w-0">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-bold text-slate-800 truncate">MacBook Air M2</div>
                  <span className="text-[10px] font-bold text-slate-400 font-mono block truncate">🟢 HOST DEVICE</span>
                </div>
              </div>
              <div className="text-right shrink-0">
                <div className="text-sm font-black text-slate-850">{laptopBattery}%</div>
                <div className="w-16 bg-slate-200 h-1.5 rounded-full mt-1.5 overflow-hidden">
            ...
            {/* WiFi Logs */}
            <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-200/50 min-w-0">
              <div className="flex items-center gap-2 text-xs font-bold text-slate-500 font-mono min-w-0">
                <Wifi className="w-4 h-4 text-blue-500 shrink-0" />
                <span className="truncate">WIFI: KOS_ANAK_AMBIS_5G</span>
              </div>
              <span className="text-[10px] font-black bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded border border-emerald-200 font-mono uppercase shrink-0">
                {wifiSignal}
              </span>
            </div>
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          <div className="bg-slate-50/70 border border-slate-200/60 p-5 rounded-2xl flex flex-col gap-4 transition-all duration-300">
            ...
            <div className="flex items-center justify-between gap-3 min-w-0">
              <div className="min-w-0">
                <span className="text-[10px] font-bold text-slate-400 font-mono block truncate">BTC / USDT (BINANCE)</span>
                <div className="text-base md:text-lg font-black text-slate-900">${btcPrice.toLocaleString()}</div>
              </div>
              <span className={`text-[10px] font-black px-2 py-0.5 rounded font-mono flex items-center gap-0.5 shrink-0 ${btcChange >= 0 ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' : 'bg-rose-100 text-rose-700 border border-rose-200'}`}>
            ...
            {/* Fear & Greed bar */}
            <div>
              <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono mb-1.5">
            ...
            <div className="pt-3 border-t border-slate-200/50 flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="w-1/2 bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs font-semibold text-slate-800 focus:outline-none focus:border-brand-500"
          ...
          <div className="bg-slate-50/70 border border-slate-200/60 p-5 rounded-2xl flex flex-col gap-4 transition-all duration-300">
            ...
            <form onSubmit={handleAddPackage} className="flex gap-2">
              <input 
                ...
                className="flex-grow bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-semibold text-slate-800 focus:outline-none focus:border-brand-500 min-w-0"
              />
              <button 
                type="submit" 
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold cursor-pointer shrink-0 flex items-center gap-1.5"
              >
            ...
            <div className="flex flex-col gap-3 max-h-56 overflow-y-auto pr-1">
              {packages.map(pkg => (
                <div key={pkg.id} className="p-4 bg-white border border-slate-100 rounded-xl flex flex-col gap-2.5 shadow-sm transition-all duration-200 hover:border-slate-200/80">
                  <div className="flex justify-between items-center gap-3 min-w-0">
                    <span className="text-xs md:text-sm font-black text-slate-800 truncate min-w-0 flex-grow" title={pkg.name}>
                      {pkg.name}
                    </span>
                    <span className={`text-[9px] font-black px-2.5 py-0.5 rounded font-mono uppercase whitespace-nowrap shrink-0 ${pkg.status === 'DELIVERED' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-amber-100 text-amber-700 border border-amber-200 animate-pulse'}`}>
                      {pkg.status}
                    </span>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] text-slate-400 font-semibold font-mono border-t border-slate-100 pt-2 min-w-0">
                    <span className="truncate">{pkg.courier} • {pkg.code}</span>
                    <span className="text-slate-650 font-bold bg-slate-50 border border-slate-250/20 px-2 py-0.5 rounded self-start sm:self-auto truncate">{pkg.lastLocation}</span>
                  </div>
                </div>
              ))}
            </div>
